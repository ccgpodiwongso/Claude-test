-- ============================================================
-- Quotr: Initial database migration
-- ============================================================
...
-- COMPANIES: user can access their own company
CREATE POLICY "company_access" ON companies
  FOR ALL USING (
    id = (SELECT company_id FROM users WHERE id = auth.uid())
  );

-- USERS: user can access users in the same company
...
-- QUOTE_LINES (chain through quotes)
CREATE POLICY "quote_line_access" ON quote_lines
  FOR ALL USING (
    quote_id IN (
...
-- QUOTE_EVENTS (chain through quotes)
CREATE POLICY "quote_event_access" ON quote_events
  FOR ALL USING (
    quote_id IN (
      SELECT id FROM quotes
      WHERE company_id = (SELECT company_id FROM users WHERE id = auth.uid())
...
-- INVOICE_LINES (chain through invoices)
CREATE POLICY "invoice_line_access" ON invoice_lines
  FOR ALL USING (
    invoice_id IN (
...
-- AVAILABILITY_SLOTS
CREATE POLICY "availability_slot_access" ON availability_slots
  FOR ALL USING (
    company_id = (SELECT company_id FROM users WHERE id = auth.uid())
  );

-- BILLING_EVENTS
CREATE POLICY "billing_event_access" ON billing_events
  FOR ALL USING (
    company_id = (SELECT company_id FROM users WHERE id = auth.uid())
  );
